package com.vshow.eshop.datasync.rabbitmq;
...
import com.vshow.eshop.datasync.service.ProductService;
    ...
    private void processBrandDataChangeMessage(JSONObject messageJSONObject) {
    	...
    	if("add".equals(eventType) || "update".equals(eventType)) { 
    		...
    		if(brandDataChangeMessageList.size() >= 2) {
    			...
    			JSONArray brandJSONArray = JSONArray.parseArray(productService.findBrandByIds(ids));
    	...
    	public void run() {
    		while(true) {
    			if(!dimDataChangeMessageSet.isEmpty()) {
    				System.out.println("【内存中有"+ dimDataChangeMessageSet.size() + "条数据需要发送】,分别是：");
